...
- **`SettingsRepository`** — DataStore-backed user preferences. Currently only stores `contactSearchEnabled: StateFlow<Boolean>` (default false). The toggle gates contact search and contact recents in `SearchViewModel`; `SettingsActivity` flips the flag and requests `READ_CONTACTS` when the user enables it.

### UI (`ui/`)

- **`SearchScreen`** — a `BottomSheet` (custom wrapper around `ModalBottomSheet`) containing `SearchField`, an `AppList` (4 fixed slots), and either history (`WebSearchList` + `ContactRecentList`) or live results (`WebSearchList` + `ContactResultList`) depending on whether the query is blank. A gear `IconButton` sits at the bottom-right of the recents state and launches `SettingsActivity`. Long-pressing an `AppItem` opens a Material 3 `DropdownMenu` with **App info** (`Settings.ACTION_APPLICATION_DETAILS_SETTINGS`), **Add to home screen** (`ShortcutManager.requestPinShortcut`) and **Don't suggest app** (`HiddenAppsRepository.hide`).
...
- **`SettingsActivity` + `ui/settings/SettingsScreen`** — opaque activity (regular `Theme.PixelishSearch`) with a Material 3 `LargeTopAppBar`, "General" section header, and a single rounded `ListItem` "Search contacts" with a trailing `Switch`. Effective state = `contactSearchEnabled && hasReadContacts`. Toggling on requests `READ_CONTACTS` if missing; the launcher's `granted` callback flips the setting on success.
- Subpackages: `ui/app/` (AppList, AppItem), `ui/contact/` (ContactResultList, ContactRecentList, ContactUtils, ContactAvatar), `ui/websearch/` (WebSearchList, WebSearchRow), `ui/bottomsheet/` (BottomSheet wrapper), `ui/settings/` (SettingsScreen), `ui/theme/`.
